...
app.post(
    ...
    async (req, res) => {
        ...
        if (!conversation || !Array.isArray(conversation)) {
            res.status(400).json({
                Message: "Percakapanmu harus valid bro!",
                data: null,
                success: false
            });
            return;
        ...
        const conversationIsValid = conversation.every((message) => {
            ...
            // kondisi kedua -- massage harus berupa object, namun bukan array!
            if (typeof message !== 'object' || Array.isArray(message)) return false;

            // kondis ketiga -- message harus berisi hanya role dan text
        ...
        try {
          ...
          res.status(200).json({
            ...
            res.status(500).json({
                success: false,
                data: null,
                Message: e.message || "Ada masalah di server Bro!"
            })
        }
    }
);
